...
bool Engine::Initialize(HINSTANCE hInstance, std::string window_title, std::string window_class, int width, int height) 
{
	...
	gfx.terrainBuffer = &terrainBuffer;
	
	gfx.InitializeTerrain(sceneManager.terrainBuffer);
...
void Engine::Update() {
...

	{
		...
		if (keyboard.KeyIsPressed('C')) {
			DirectX::XMVECTOR lightPosition = this->gfx.Camera3D.GetPositionVector();
...
void Engine::RenderFrame()
{
	...
	gfx.RenderFrame();
	//gfx.DebugDrawTest();
	gfx.RenderColliderDebug(&physicsBuffer);
	gfx.SwapBuffer();
}
