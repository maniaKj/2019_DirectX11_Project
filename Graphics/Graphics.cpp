...
#pragma region CheckFrame_
	...
	float dt = fpsTimer.Time.GetDeltaTime();
	fpsTimer.Tick();
	if (fpsTimer.GetMilisecondsElapsed() > 1000.0) {
		fpsString = "FPS: " + std::to_string(fpsCounter) + ", deltaTime : " + std::to_string(fpsTimer.Time.GetDeltaTime());
		fpsCounter = 0;
		fpsTimer.Restart();
	}
	spriteBatch->Begin();
...
#pragma region RENDER_ImGUI
	...
	ImGui::DragFloat4("orientation", &gameObjBuffer->at(2)->GetComponent<BoxCollider>()->debugModel.Orientation.x, 0.1f, -90.0f, 90.0f);
