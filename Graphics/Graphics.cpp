...
DirectX::XMFLOAT3 QuatenionToEuler2(float x, float y, float z, float w) {
	

	//float roll;
	//float pitch;
	//float yaw;
	//float radian = 360 / 6.28;

	////x *= radian; y *= radian; z *= radian;

	//float mul = x * y + z * w;
	//pitch = asin(2 * mul);

	//if (fabsf(0.5 - mul) < 0.01) {
	//	roll = 2 * atan2(x, w);
	//	yaw = 0.0f;
	//}
	//else if (fabsf(0.5 + mul) < 0.01) {
	//	roll = -2 * atan2(x, w);
	//	yaw = 0.0f;
	//}
	//else {
	//	roll = atan2(2 * y*w - 2 * x*z, 1 - 2 * y * y - 2 * z * z);
	//	yaw = atan2(2 * x*w - 2 * y*z, 1 - 2 * x * x - 2 * z * z);
	//}
	...
	if (test > 0.499) { // singularity at north pole
		...
		attitude = 3.14 / 2;
		bank = 0;
		return DirectX::XMFLOAT3(heading, attitude, bank);
	}
...
void GraphicsManager::RenderFrame()
{
	...
	*/
	...
	
	{
		...
		for (std::vector<GameObject_v2*>::iterator it = gameObjBuffer->begin(); it != gameObjBuffer->end(); it++) {
			bool Render_enable = (*it)->renderer.enabled;
			bool GameObject_enable = (*it)->enabled;
			if (GameObject_enable && Render_enable) {
				deviceContext->IASetInputLayout(this->vert_Shader_Buffer[(*it)->renderer.vertexshaderID]->GetInputLayout()); //IA에 입력할 배치 적용
				deviceContext->VSSetShader(this->vert_Shader_Buffer[(*it)->renderer.vertexshaderID]->GetShader(), NULL, 0); //그릴 때 쓸 셰이더 적용
				deviceContext->PSSetShader(this->pixel_Shader_Buffer[(*it)->renderer.pixelshaderID]->GetShader(), NULL, 0); //그릴 때 쓸 셰이더 적용
...
#pragma region RENDER_ImGUI
	...
	GameObject_v2 * gameObjPick = gameObjBuffer->at(0);
	gameObjPick->transform.UpdateMatrix();
	...
	GameObject_v2 * gameObjPick2 = gameObjBuffer->at(1);
	ImGui::Begin("GameObject Compo Test");
	if (ImGui::Button("Add BoxCollider")) gameObjPick2->AddComponent<BoxCollider>();
	if (ImGui::Button("Remove BoxCollider")) gameObjPick2->RemoveComponent<BoxCollider>();
	ImGui::End();

	static DirectX::XMFLOAT3 baseEuler = DirectX::XMFLOAT3(0.0f, 0.0f, 0.0f);
	DirectX::XMFLOAT4 midQuaternion;
	DirectX::XMStoreFloat4(&midQuaternion, DirectX::XMQuaternionRotationRollPitchYaw(baseEuler.x, baseEuler.y, baseEuler.z));
	DirectX::XMFLOAT3 finalEuler = QuatenionToEuler2(midQuaternion.x, midQuaternion.y, midQuaternion.z, midQuaternion.w);
...
void GraphicsManager::InitializeSimpleGeometry(ModelBuffer & _modelBuffer)
{
	...
	model = new Model();
	if (!model->Initialize(testbox.vertices, testbox.vertexSize, testbox.indices, testbox.indexSize, this->device.Get(), this->deviceContext.Get(), this->cb_vs_vertexshader)){
	...
	model = new Model();
	if (!model->Initialize(testbox2.vertices, testbox2.vertexSize, testbox2.indices, testbox2.indexSize, this->device.Get(), this->deviceContext.Get(), this->cb_vs_vertexshader)) {
		MessageBoxA(NULL, "Model Initialize error.", ERROR, MB_ICONERROR);
		return;
	}
	_modelBuffer.buffer.push_back(model);
	...
	model = new Model();
	if (!model->Initialize(&testSphere.vertices, &testSphere.indices, this->device.Get(), this->deviceContext.Get(), this->cb_vs_vertexshader)) {
		MessageBoxA(NULL, "Model Initialize error.", ERROR, MB_ICONERROR);
		return;
	}
	_modelBuffer.buffer.push_back(model);
	...
	model = new Model();
	if (!model->Initialize(&testCylinder.vertices, &testCylinder.indices, this->device.Get(), this->deviceContext.Get(), this->cb_vs_vertexshader)) {
		MessageBoxA(NULL, "Model Initialize error.", ERROR, MB_ICONERROR);
		return;
	}
	_modelBuffer.buffer.push_back(model);
	...
	model = new Model();
	if (!model->Initialize(testplane.vertices, testplane.vertexSize, testplane.indices, testplane.indexSize, this->device.Get(), this->deviceContext.Get(), this->cb_vs_vertexshader)) {
		MessageBoxA(NULL, "Model Initialize error.", ERROR, MB_ICONERROR);
		return;
	}
	_modelBuffer.buffer.push_back(model);
...
bool GraphicsManager::InitializeTerrain(TerrainModelBuffer & _terrainmodelBuffer)
{
	for (std::vector<std::shared_ptr<Terrain>>::iterator it = terrainBuffer->begin(); it != terrainBuffer->end(); it++) {
		...
		model->Initialize(&desc.vertexBuffer, &desc.indexBuffer, this->device.Get(), this->deviceContext.Get(), this->cb_vs_vertexshader);
		_terrainmodelBuffer.buffer.push_back(model);
		(*it)->gameObject->renderer.SetModel(model);
		(*it)->gameObject->GameObjectName = "";
	}
	return true;
}
...
void GraphicsManager::Find_Shader_File(std::wstring & shaderfolder) //나중에 쉐이더 초기화 함수와 합칠 것
{
	//폴더 안에 hlsl파일 확인 후 
	...
	*/
	...
	if ((handle = _findfirst(vertexShader_path.c_str(), &fd)) != -1L) {
		do {
			...
			if (!vs->Initialize(this->device, shaderfolder + StringHelper::StringToWide(shader_filename + ".cso"), layout3D_skinned, numElements_skinned, shader_filename))
	...
	if ((handle = _findfirst(geometryShader_path.c_str(), &fd)) != -1L) {
		do {
			...
			if (!gs->Initialize(this->device, shaderfolder + StringHelper::StringToWide(shader_filename + ".cso"), shader_filename))
				MessageBoxA(NULL, "Shader Initialize error.", ERROR, MB_ICONERROR);
	...
	if ((handle = _findfirst(path.c_str(), &fd)) != -1L) {
		do {
			if (StringHelper::GetFileExtension(fd.name) == "fbx") {
				...
				Model *model = new Model();
				if (!model->Initialize(filePath + fd.name, this->device.Get(), this->deviceContext.Get(), this->cb_vs_vertexshader, this->cb_vs_boneData, animClipBuffer)) {
					MessageBoxA(NULL, "Model Initialize error.", ERROR, MB_ICONERROR);
					return;
				}
